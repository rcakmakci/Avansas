...
function renderCart(products) {
    ...
    if (!products.length) {
        ...
        let cartTotalQuantity = calculateTotalQuantity(products);

        var cartDetail = {
            "totalPrice": cartTotalPrice,
            "productLength": cartTotalQuantity
        }
...
function calculateTotalQuantity(products) {
    return products.reduce((total, product) => {
        if (product.Adet <= 0) {
            console.error('Ürün adedi negatif veya sıfır olamaz:', product);
            return total;
        }
        return total + product.Adet;
    }, 0);
}
